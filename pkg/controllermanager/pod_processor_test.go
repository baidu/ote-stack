...
func TestGetCreateOrUpdatePod(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:            "An existent pod should error when create a new pod fails.",
			pod:             testPod1,
...
func TestDeletePod(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:          "An existent pod should delete in etcd.",
			pod:           testPod,
			getPodResult:  nil,
			errorOnDelete: nil,
			expectActions: []kubetesting.Action{
		...
		{
			name:          "An existent pod should error when delete a pod fails.",
			pod:           testPod,
			getPodResult:  nil,
